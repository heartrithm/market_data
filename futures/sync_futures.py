...
def get_sync_futures_class(exchange, symbol, interval=None, start=None, end=None, host=None, data_type="futures"):
    """ Abstraction layer that returns an instance of the right SyncRates class """
    if exchange.lower() == "ftx":
        # FTX only supports 1h
        return SyncFTXFutures(symbol, "1h", start=start, end=end, host=host, data_type=data_type)
    ...
    def pull_data(self):
        if self.data_type == "futures":
            ...
            self.sync(
                ...
                end_format="end_time",
                timestamp_units="s",  # that we get from the exchange
                result_key="result",
