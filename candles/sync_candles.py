...
def get_sync_candles_class(exchange, symbol, interval, start=None, end=None, force_range=None):
    ...
    if exchange.lower() == "bitfinex":
        return SyncBitfinexCandles(symbol, interval, start, end, force_range)
...
class BaseSyncCandles(object):
    ...
    def __init__(self, symbol, interval, start=None, end=None, force_range=None):
        self.symbol = symbol
    ...
    def get_iterations_for_range(self, batch_limit):
        """ Given a start, end timstamp, return the incremental steps required to fill in data, for
            ...
            This will only pull data from the latest in the db, until now(). If you're expanding the start time
            further back in time, you have drop data and start over.
        """
    ...
    def sync(self, endpoint, extra_params={}, extra_tags=None):
        ...
        # If force_range is set, never check the DB for data, nor change the ranges. JFDI.
        if (self.start and not self.end) and not self.force_range:
            self.end = datetime.datetime.now().timestamp()

        if (not self.start and not self.end) and not self.force_range:
            now = datetime.datetime.now()
            self.start = (now - datetime.timedelta(days=self.DEFAULT_SYNC_DAYS)).timestamp()
            self.end = now.timestamp()

        steps, start, end = self.get_iterations_for_range(self.API_MAX_RECORDS)
